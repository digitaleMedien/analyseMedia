...
getResultPages <- function(mediaName,searchTerm) {
  ...
  write_xml(doc,paste0(sysFile("data/resultpages"),mediaName,"_","resultpage_",gsub(" ","_",searchTerm,fixed=TRUE),"_page_1.htm"))
...
getPages <- function(url,mediaName,searchTerm,pageCount) {
  ...
  doc <- read_html(url)
  write_xml(doc,paste0(sysFile("data/articlepages"),mediaName,"_","articlepage_",gsub(" ","_",searchTerm,fixed=TRUE),"_page_",pageCount,".htm"))
}
...
analysePage <- function(mediaName="fazBlog",loadedPage) {
  ...
  if (length(grep("resultpage_",loadedPage))==1) {
    ...
    doc <- read_html(paste0(sysFile("data/resultpages"),loadedPage),encoding = "UTF-8")
  ...
  if (length(grep("articlepage_",loadedPage))==1) {
    ...
    doc <- read_html(paste0(sysFile("data/articlepages"),loadedPage),encoding = "UTF-8")
