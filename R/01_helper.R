...
#' 
#' @export
sysFile <- function(folder,file=NULL){
  system.file(folder, package = "AnalyseMedia") %p0% "/" %p0% file 
}
...
getResultPagesHelper <- function(mediaName,searchTerm,pageCount) {
  ...
  doc <- read_html(qry)
  write_xml(doc,paste0(sysFile("data/resultpages"),mediaName,"_","resultpage_",gsub(" ","_",searchTerm,fixed=TRUE),"_page_",pageCount,".htm"))
}
